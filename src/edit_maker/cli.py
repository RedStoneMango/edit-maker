...
def parse_args():
    ...
    parser.add_argument("--intro-duration", help="Specifies the length of the edit intro. The intro is sped up / down in such a way that its end (the edit's start) directly aligns with the provided audio timestamp. If this option is not provided, the intro's natural length is used. Has no effect if no --intro is provided", required=False, default=None, type=timestamp_type, metavar="[SECOND|SECOND.FRACTION|MINUTE:SECOND|...]")
    parser.add_argument("--intro-audio-overlap", help="Enables the edit audio to start playing while the intro is still active. Can be used in combination with --intro-duration to schedule the switch from intro to main edit to a peak of the audio", required=False, default=None, action="store_true")
