...
def parse_args():
    ...
    generate_parser.add_argument("--beat-deviation", help="The allowed deviation between the first detected beat duration and the second beat duration. If the first clip's duration exceeds this value relatively to the second beat duration, it is flattened to prevent a flashing first graphic in case the audio starts shortly before the first logical beat", type=float, default=0.5, required=False)
